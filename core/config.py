...
class RoutingConfig(BaseModel):
    ...
    #   08-07 실측에서 코딩 서브모델이 테스트 작성 범주만 앵커보다 못했다
    #   (0~1/6 vs 1~2/6). 그 근거는 Devstral 기준이고 현재 서빙 모델
    #   (Qwen3-Coder)로는 재측정하지 않았으므로, 재측정 전까지 기존 계약을
    #   그대로 지킨다 — 측정 없이 계약을 넓히지 않는다. TODO(nexus): 재측정.
    #
