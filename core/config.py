...
class RoutingConfig(BaseModel):
    ...
    # 코딩 질의 정규식 — 키워드만으로는 못 잡는 "구현/추가" 형태를 위해 추가(2026-08-20).
    #
    # [왜 키워드가 아니라 정규식인가]
    #   coder_keywords 는 부분 문자열 매칭이다. "구현" 을 넣으면 "구현 원리를
    #   설명해줘" 같은 지식 질의까지 걸리고, "만들어" 를 넣으면 "보고서 만들어줘"
    #   처럼 코딩과 무관한 요청이 코딩 모델로 샌다. NOVA 는 코딩 전용 도구가
    #   아니라 일반 업무 질의도 받으므로 이 오탐은 실제 피해가 된다.
    #   그래서 routing.tool_regex_patterns 와 같은 방식으로 "동사 + 코딩 명사"를
    #   요구한다(실측: 구현요청 12/12 포착, 지식질의 오탐 0/13).
    #
    # [테스트 작성은 제외한다 — coder_exclude_patterns 로 강제]
    #   08-07 Devstral 실측에서 이 범주만 앵커보다 못했다(0~1/6 vs 1~2/6).
    #   2026-08-21 현 서빙 모델(Qwen3-Coder)로 재측정했고 **결론은 같았다.**
    #     과제: 세 함수용 pytest 작성 → 생성한 테스트를 실제로 실행해 판정.
    #     A.X 3/3 통과 · Qwen 0/3 실패(3회 반복, 매번 같은 유형).
    #     실패 원인은 **경계 조건 오독** — 스펙이 `q < threshold` 인데
    #     `<=` 로 읽어 임계치와 같은 값을 '부족'으로 판정했다.
    #     assert 개수는 오히려 Qwen 이 많았다(40 vs 34) — 많이 쓰지만 틀린다.
    #   → 제외를 유지한다. 근거가 Devstral 기준이라는 단서는 이제 없다.
    #
    # [실측 근거 — 2026-08-20 bakeoff]
    #   "turnover_rate 함수를 추가해줘" 를 앵커가 처리하면 Write 4회가 전부
    #   구문 오류로 거부되고, 완성 코드를 답변에 10,723자 쏟아냈다(파일 미변경).
    #   같은 과제를 코딩 모델로 보내면 Edit 3회로 함수+테스트를 넣고 6 passed.
    #   기본값은 비어 있다 — 실제 패턴은 config/*.yaml 에 둔다
    #   (routing.tool_regex_patterns 와 같은 관례). 그래야 운영자가 키워드를
    #   좁히면 그 의도대로 좁아지고, 배포별로 다르게 조정할 수 있다.
    coder_regex_patterns: list[str] = Field(default_factory=list)
    # 코딩 모델로 보내지 않을 질의 — 위 키워드·정규식보다 뒤, 정규식보다는 앞에서 본다.
    #   순서가 중요하다: 명시 키워드(디버깅·리팩터링 등 동률이 확인된 범주)는
    #   그대로 통과시키고, 그 다음에 이 제외를 적용한 뒤, 마지막에 정규식을 본다.
    #   그래야 "테스트 코드 디버깅해줘"(디버깅=검증된 범주)는 코딩 모델로 가고,
    #   "이 함수 테스트 코드 짜줘"(순수 테스트 작성)는 앵커에 남는다.
    #   "함수를 추가하고 테스트도 추가해줘" 처럼 구현이 주된 요청은 걸리지 않는다.
    coder_exclude_patterns: list[str] = Field(
        default_factory=lambda: [
            r"테스트\s*코드",
            r"단위\s*테스트",
            r"unit\s*test",
