...
class ImageGenerateTool(BaseTool):
    ...
    async def call(self, input_data: dict[str, Any], context: ToolUseContext) -> ToolResult:
        ...
        # 모델이 사용자에게 그대로 제시할 수 있도록 마크다운 이미지/링크를 본문에 포함한다.
        # (download_url 등 구조화 메타데이터도 함께 넘겨, 웹이 신뢰할 값은 모델 텍스트가
        #  아니라 이 메타데이터에서 뽑아 쓰도록 한다 — DocumentExport와 동일 원칙.)
        return ToolResult.success(
            f"이미지를 생성했습니다: {filename} ({size_bytes:,} bytes, {out_width}x{out_height}).\n"
            f"사용자에게 아래 미리보기/다운로드 링크를 그대로 제시하세요:\n"
            f"![{filename}]({preview_url})\n"
