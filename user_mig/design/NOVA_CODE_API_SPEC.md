...
(함수|클래스|모듈|스크립트|컴포넌트|엔드포인트|API|UI|화면|페이지|기능|창)[^\n]{0,15}?(만들|작성|생성|추가|수정|짜)
코드\s*\S{0,6}\s*(짜|작성|만들|생성|수정)
```
...
- **걸리지 않는 흔한 예**: "타입 힌트를 **붙여**줘"(동사 미포함), "이름을 **바꿔**줘".
  이런 요청은 `"... 함수를 수정해줘"` 처럼 목록에 있는 동사로 바꾸면 전환된다.
...
이 경우 정확 매칭은 당연히 실패하고, **위 폴백도 구제하지 못한다** — 폴백이 줄 단위
비교라 마지막 줄이 다르면 매치가 아니기 때문이다.

**개선 여지(플러그인 측):** 마지막 줄에 한해 **접두 일치(prefix match)** 를 허용하면
이 유형이 구제된다. 단 그만큼 오적용 위험이 커지므로, 구제한 경우 **MUST** 사용자에게
diff 를 보여 확인받는다. 코딩 모델로 라우팅되면 이 유형이 거의 사라진다(실측 100% 정확).

### 9.4 실패 시 처리 순서

1. `applyChange` 가 `null` → 사용자에게 **diff 후보를 보여주고 수동 확인**을 요청한다.
2. 자동 재시도는 **1회까지**. 재시도 시 프롬프트에 실패한 `find` 를 인용하고
   "이 조각은 파일에 없다. 다시 정확히 복사하라"를 덧붙인다.
