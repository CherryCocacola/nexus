...
Claude Code의 아키텍처를 Python으로 재구현하여, A.X-4.0(72B)을 중심으로
...
- **모델**: A.X-4.0 72B FP8 (primary), ExaOne 7.8B (Korean auxiliary), multilingual-e5-large (embedding), Gemma 계열 VLM (vision), SD 3.5 Large (image)
  - 세그먼트별 primary 대체: 대학=Gemma / 국가 PoC=ExaOne / 일반기업=Qwen
- **GPU**: 2×B200 (현행 배포, TP=2) — RTX 5090(32GB)은 개발 베이스라인, H200/GB10 세그먼트 확장 경로
