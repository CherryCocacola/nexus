    ...
    try:
        ...
        web_engine = QueryEngine(
            ...
            system_prompt=(
                ...
                "- Document processing: Delegate PDF/DOCX/XLSX/HWP/PPT analysis to scout sub-agent\n"
                "- Code assistance: Review, explain, and edit code\n\n"
                "## File handling rules (STRICT)\n"
                "Binary document files (.pdf, .docx, .doc, .xlsx, .xls, .hwp, .pptx, .ppt):\n"
                "  - You do NOT have DocumentProcess tool. Scout does.\n"
                "  - You MUST call Agent(prompt='분석 지시', subagent_type='scout') "
                "passing the file path.\n"
                "  - NEVER call Read on binary document files — it returns garbage and wastes turns.\n"
                "  - Trust the Scout summary completely. Do NOT retry with Read after Scout.\n"
                "Text / code files (.py, .md, .txt, .yaml, .json, ...):\n"
                "  - Use Read directly.\n\n"
                "When Scout returns a summary, present it to the user in Korean "
                "(or the user's language). Do not add your own analysis of the raw file — "
                "Scout already did that work.\n\n"
